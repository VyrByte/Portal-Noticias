...
### Requisitos Previos
Asegúrate de tener instalado Node.js (versión 16 o superior) y npm en tu sistema.
...
* **Interacciones Clásicas:** Los titulares se subrayan sutilmente al pasar el mouse, los botones son planos y de alto contraste, y las insignias de categorías emulan etiquetas de secciones impresas sin colores fluorescentes.
* **Iconografía Vectorial Pura:** Todos los iconos de la interfaz son vectores SVG cargados directamente en línea, garantizando una estética limpia y sin recurrir a emojis unicode.
...
│   └── package-lock.json
├── frontend/
│   ├── index.html           (Punto de entrada HTML con SEO y fuentes)
│   ├── vite.config.js       (Proxy de API a http://localhost:3001)
│   ├── package.json         (Dependencias del cliente con Vue 3, Vuex, Router, Axios)
│   └── src/
...
│       ├── store/
│       │   └── index.js     (Estado global Vuex con CRUD de favoritos y noticias)
│       ├── utils/
...
│       ├── components/      (7 componentes interactivos con vectores SVG)
│       │   ├── NavBar.vue
│       │   ├── CategoryFilter.vue
│       │   ├── NewsCard.vue
│       │   ├── SearchBar.vue
